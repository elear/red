<template>
  <DialogRoot v-model:open="isFeatureFlagsModalVisible">
    <DialogPortal>
      <DialogOverlay class="bg-blue-900/50 data-[state=open]:animate-overlayShow fixed inset-0 z-100 overflow-y-scroll py-16">
        <DialogContent
          ...
          <DialogTitle class="text-xl font-bold py-2 pr-16">
            RFC-Editor.org Feature Flag Experiments
          </DialogTitle>
          <DialogDescription as="div" class="w-full leading-6 pb-3 flex flex-row border-b-1 border-b-gray-300 dark:border-b-gray-600">
            <p class="flex-2 text-balance">
              <b class="text-red-800 dark:text-red-400">Warning: </b> This config panel is intended for a specific
              audience. It is for those
            ...
            <div
              ...
              <p class="text-balance mt-1">
                ...
                and they may <b>or may not</b> become default in the future.
              </p>
            </div>
          </DialogDescription>
          ...
          <DialogClose class="absolute border-1 border-gray-400 top-4 right-5 rounded cursor-pointer px-3 py-1 focus:bg-gray-500/25 hover:bg-gray-500/25">
            Close
          </DialogClose>
        </DialogContent>
      </DialogOverlay>
